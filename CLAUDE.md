...
**This is the source of truth.** [AGENTS.md](AGENTS.md) mirrors this file — symlinked to it on the maintainer's machines — so Codex, which reads `AGENTS.md`, sees the same content. Edit **CLAUDE.md**; AGENTS.md follows. Never edit AGENTS.md directly. (This repo tracks AGENTS.md as a thin pointer rather than a literal symlink because Git for Windows has `core.symlinks=false`.)
...
2. **Working *on* the bundle** means authoring/editing skills and playbooks, keeping the three top-level docs in sync, and re-deploying changed skills to the install root. There is no app to build and no test suite — the "product" is the prose in the skills.
...
| `scripts/install.{ps1,sh}` + OS wrappers | Installers for the agent config roots |
| `README.md` | Human-facing overview + workflow graph + skill catalog |
...
| `CLAUDE.md` / `AGENTS.md` | These files — instructions for working on the bundle. **CLAUDE.md is canonical; AGENTS.md mirrors it (symlinked on the maintainer's setup).** |
...
`skills/` here is the **source of truth**, but editing a skill does nothing until it's copied to the install location an agent actually reads:

- **Local — one target: `~/.claude/skills/`.** This machine has `~/.agents/skills/` symlinked to it (and `.agents` is what Codex reads), so deploying once to `~/.claude/skills/` covers both agents. There is no separate `.codex` deploy.
- **Remote machines in use**, e.g. sbot2 at `\\sbot-2\C\Users\bot\.claude\skills\` (SMB) or `scp` to `sbot2:C:/Users/bot/.claude/skills/`.

After deploying, the target agent must restart to reload skills.

The installer (`scripts/install.ps1`) is **all-or-nothing on conflicts** — it refuses the whole run if any skill already exists. For a single changed/new skill, copy that one folder directly (`Copy-Item`/`scp` into `~/.claude/skills/<name>/`) rather than `-Overwrite`-ing the world.

### Keep the docs in sync

When you add, rename, or materially change a skill, update **all three** in the same commit: `README.md` (catalog row + the workflow graph if the skill set changed), `WORKFLOW.md` (narrative), and the relevant `INSTALL.md` notes. A new skill also needs registering in the skill catalog above.
...
- **Goal output is one unbroken line, nothing else** — no preamble, no command line, no internal line breaks, so it copy-pastes in one piece.
- **Workflow commands in goal text default to `$` form** (`$start-session`, `$wrap`, `$audit`, `$end-session`, Codex-style), each `$` token space-flanked — never touching a comma/period/semicolon or Codex won't parse it. Use `/` form only when the goal targets Claude Code (the user asks "for claude"). MCP paths and shell commands keep their real syntax.
...
- **Editing AGENTS.md directly.** It mirrors this file (symlinked on the maintainer's setup); edit CLAUDE.md.
