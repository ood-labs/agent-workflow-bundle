...
The generated condition encodes the phase lifecycle, not just the code: orient the way `/start-session` does, work each sub-phase in order against its pass criteria, write the devlog, and close out the way `/end-session` does (devlog `status: complete`, `approval: pending`, state.md updated, committed). It keeps humans on the boundaries: destructive/irreversible steps (a confirmed `git reset`, a force-push) are carved out as one-time prerequisites the loop verifies but never runs, and human gates (aesthetic sign-off, ratification, review) are stop points the loop surfaces evidence for but never crosses or self-approves. So each goal is scoped to a single gate-to-gate span — usually one phase, generated one at a time, with the next produced after the prior gate clears. Every goal also bakes in a stop-and-ask-the-human rule for unforeseen blockers (irreversible/ambiguous/missing-dependency/repeatedly-failing situations) plus a turn cap. Because the `"slash goal"` evaluator only judges against what the agent surfaces in the transcript, the condition requires the agent to paste its proof (test output, gate evidence, commit line, devlog frontmatter). The skill returns the goal inline in chat (never a file); you run the command.
