...
The skill drives **as far as you ask** — one phase or a whole overnight run — and its central job is gate handling. An autonomous loop that stops at every sign-off is useless overnight, so the skill sorts each gate into one of three tiers: **self-serve** (aesthetic/visual checks and the per-sub-phase approval cadence → run the check, log the verdict + screenshot, commit `approval: pending`, and continue), **conditional-proceed** (a real decision pre-authorized with a testable rule the loop can evaluate from its own output — e.g. "accept the dependency if `cargo deny` passes and the license is MIT/Apache, else stop"), and **hard-stop** (the genuinely irreversible or unsettled — git history rewrite, editing the spec, a core/upstream fork, a failed pre-authorized condition, tools down after the semantic fallback, or a decision the plan doesn't settle). Keeping the human in the loop happens at *authoring time*: the skill elicits the pre-authorizations up front so the loop doesn't stall at 2am. The condition still encodes the full per-slice lifecycle (orient, prove against pass criteria via the project's testability contract, devlog, commit) and, because the evaluator only judges what's surfaced in the transcript, requires the agent to paste its proof. The skill returns the goal inline in chat (never a file); you run the command.
